...
-- Triggers for updated_at (using DO block to avoid errors if already exists)
DO $$ BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_live_state_updated_at') THEN
